...
const handleDeleteRestaurant = async (event) => {
  try {
    const id = event.currentTarget.getAttribute('data-id'); // Use currentTarget to get the button
    if (id) {
      // First, delete all dishes associated with this restaurant
      const dishesCollection = collection(db, "dishes"); // Assuming dishes collection is called "dishes"
      const q = query(dishesCollection, where("restaurantId", "==", id)); // Assuming each dish has a field "restaurantId"
      const querySnapshot = await getDocs(q);

      // Delete each dish
      querySnapshot.forEach(async (dishDoc) => {
        const dishRef = doc(db, "dishes", dishDoc.id);
        await deleteDoc(dishRef);
      });

      // After all dishes are deleted, delete the restaurant
      const docRef = doc(db, "restaurants", id);
      await deleteDoc(docRef);

      loadRestaurants(); // Reload the restaurants
    } else {
      console.error('Error: No ID found for deletion');
    }
  } catch (error) {
    console.error('Error deleting restaurant and its dishes:', error);
  }
};
